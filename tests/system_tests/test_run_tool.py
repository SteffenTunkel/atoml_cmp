...
class TestRunThrough(unittest.TestCase):
    def test_atoml_cmp_usecase(self):
        num_logreg_csv = 4 * 8
        # !!! currently deactivated because overwriting leads to permission denied error on linux
        # create_wine_arff()
        ...
        # !!! currently deactivated because caret is not working properly on linux (maybe due to not overwriting)
        # concerns: VerySmall, Outlier, Zeroes
