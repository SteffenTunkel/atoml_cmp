...
public class ModelGenerator {
    ...
    public Classifier buildClassifier(Instances traindataset) {
        ...
        //c.setUseKernelEstimator(false);
        //c.setUseSupervisedDiscretization(false);
	...
	*The value used to seed the random number generator (Value should be >= 0 and and a long, Default = 0).
	...
	'i' = attribs, 'o' = classes, 't' = attribs .+ classes) for wildcard values, Default = a).
        ...
        *m.setBatchSize("1");
         */
        try {
            c.buildClassifier(traindataset);
    ...
    public ArrayList<Prediction> getPredictions(Classifier model, Instances traindataset, Instances testdataset) {
        Evaluation eval = null;
        try {
            // Evaluate classifier with test dataset
            eval = new Evaluation(traindataset);
