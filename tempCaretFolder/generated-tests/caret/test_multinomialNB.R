...
library(foreign)
library(testthat)
suppressMessages(install.packages("klaR"))
test_that("multinomialNB",{
    set.seed(825)
    ...
    paramGrid <-  expand.grid(fL = 0, usekernel = TRUE, adjust = 1.0)
    ...
    write.csv(x = csv_df,
            file = file.path("../log/pred_CARET_MultinomialNB_Uniform.csv"),
            row.names = FALSE)
